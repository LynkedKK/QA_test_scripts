...
class Main(BasePage):
  ...
  def inputTableFilter(self, filter_text):
    ...
    # dismiss the drop down hints list from filter
    self.driver.press_keycode(KEYCODE_DPAD_UP)
    sleep(0.2)
  ...
  def getCenter(self, left, top , right, bottom):
    # [36,516][1500,592]
    height = bottom-top
    width = right-left
    return (left+width/2, top+height/2)

  def tapTopMostOrder(self, device="ANDROID_TABLET"):
    # NOTES: for testing use TID_031_a.py
    if device=="DESKTOP":
      # NOTES: save this for keep working on desktop later
      # self.tapButton('//android.view.View[@id="orders"]/div[{}]'.format(idx))
      pass

    if device=='ANDROID_TABLET':
      (x, y) = self.getCenter(36,516, 1500,592)
